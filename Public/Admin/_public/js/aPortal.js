...
var href_local = window.location.href.split('/Public/Admin/')[0];

var crm = {
	api_local: href_local + '/adminApi',
	app_local: href_local + '/Public/Admin',
	default_img: href_local + '/Public/Admin/_public/images/pic-default.png',
}
	...
	layui.use('layer', function(){
		...
		$.post(url, data, function(res){
			if(res.code == '706'){
				...
				parent.location.href = crm.app_local + '/Login/login.html';
				layer.close(indexs);
			}else if(res.code == '110'){
				layer.msg(res.message);
				parent.location.href = crm.app_local + '/Index/init.html';
				layer.close(indexs);
			}else if(res.code > 0){
...
//获取地址中 ? 后面的参数
crm.getParam = function(param){ //传入需要获取的参数
	var params = param;
	param == '' ? (param = {}) : '';
	...
	var param_str = _href.split('.html?')[1];
	var param_arr = param_str.split('&');
	$.each(param_arr, function(i, val){
		var _param_value = val.split('=');
		if(params != ''){
			$.each(param, function(it, valt){
				if(_param_value[0] == it){
					param[it] = _param_value[1];
					param[it] = decodeURIComponent(param[it]); //中文转码
					return false;
				}
			});
		}else{
			param[_param_value[0]] = decodeURIComponent(_param_value[1]);
		}
	});
	// console.log(param);
	return param;
...
//随机数
crm.rands = function(min, max){
	return Math.ceil( Math.random() * (max-min) + min);
}

crm.tb = function(elem, list, cols, callback){
	// console.log(elem, list, cols)
...
crm._table = function(elem, param = {}, callback){
	layui.use(['table', 'form'], function(){
		var table = layui.table,
			form = layui.form;
		table.render({
			...
			cols: param.cols,
			// skin: 'nob',
			even: (param.even == 'undefined' ? param.even : false),
			page: !param.page,
			limit: param.limit || 30,
			limits: param.limits || [10, 30, 50, 100],
		});
		callback(table, form);
	});
}
// 上传图片
crm._upload = function(elem, param = {}, callback){
	layui.use(['element', 'upload'], function(){
		...
		upload.render({
			elem: elem,
			url: crm.api_local + "/Tool/uploadImg",
			size: 10000,
			accept: 'images',
...
// 多图上传
crm._uploadMulti = function (param = {}, callback){
	$.ajax({
		url: crm.api_local + "/Tool/uploadImg",
		type: "POST",
		data: param.formData,
...
//获取关键词
crm._keywords = function(){
	var _keywords = $("input[name='keywords']");
	var keywords = {};
	$.each(_keywords, function(key, value){
		keywords[$(this).data('key')] = $(this).val();
	});
	return keywords;
}
...
//构建搜索框
//查询数据
crm.searchBox = function(searchDoom, table_name, searchValue={}, doSearch=''){
	crm.post(crm.api_local + "/Portal/index", {table_name: table_name}, function(data){
		crm.searchBoxHtml(searchDoom, data.dict_columns, searchValue={}, doSearch);
	});
}
//构建搜索框
crm.searchBoxHtml = function(searchDoom, columns, searchValue={}, doSearch=''){
	var _search = '';
	_search += `
		<div class="flex-wrap">`;
	...
	$.each(columns, function(i, val){
		var _source = '';
		if(val.source_info != ''){
			_source_list[val.columns_name] = val;
			_source = `
					<button class="layui-btn layui-btn-sm layui-btn-new sp-img" onclick="crm.open_portal('` + val.source_info.source_table_name + `', '` + val.source_info.source_columns_name + `', '` + val.columns_name + `');">
		...
		}else if(val.type == 'checkbox' || val.type == 'select'){
			// html_input += `
			// 		<select class="layui-input" name="` + i + `" lay-search>`;
	...
	_search += `
		...
		<div style="text-align:center;">
			<button class="layui-btn layui-btn-new" onclick="` + doSearch + `">搜索</button>
		</div>`;
	if(searchDoom != ''){
		if(searchDoom.html() == ''){
			searchDoom.html(_search);
		}
	}
	return _source_list;
};
crm.open_portal = function(source_table_name, source_columns_name, columns_name, columns_name_value=''){
	var href = crm.app_local + `/Portal/index.html?table_name=` + source_table_name;

	if(columns_name_value == ''){
		columns_name_value = $("#" + columns_name).val();
	}
	if(columns_name_value){
		href += '&' + source_columns_name + '=' + columns_name_value;
	}
	layer.open({
		title: '查看',
		type: 2,
		...
		yes: function(index){
			...
			$.each(res, function(key, value){
				data_str += value[source_columns_name] + ';';
			});
			data_str = data_str.substring(0, data_str.lastIndexOf(';'));

			$("#" + columns_name).val(data_str);
