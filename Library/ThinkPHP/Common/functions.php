...
function session($name='',$value='') {
    ...
    //         session_id($name['id']);
    //     }
    //如果设置了VAR_SESSION_ID 则从对应的http头中取前端定义的 session_id();
    if(C('VAR_SESSION_ID') && isset( $_SERVER['HTTP_' . strtoupper(C('VAR_SESSION_ID'))] ) && $_SERVER['HTTP_' . strtoupper(C('VAR_SESSION_ID'))] != ''){
        session_id($_SERVER['HTTP_' . strtoupper(C('VAR_SESSION_ID'))]);
    }elseif(C('VAR_SESSION_ID') && isset($_REQUEST[C('VAR_SESSION_ID')])){
        session_id($_REQUEST[C('VAR_SESSION_ID')]);
    }
    if(is_array($name)) { // session初始化 在session_start 之前调用
        if(isset($name['prefix'])) C('SESSION_PREFIX',$name['prefix']);
